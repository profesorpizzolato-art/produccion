...
def generar_certificado_pdf(nombre, dni, puntaje):
    ...
    texto_cert = f"Por haber aprobado satisfactoriamente la evaluacion IPCL MENFA 3.0 con {puntaje}/240 puntos."
...
def evaluacion():
    ...
    if st.session_state.examen_calculado:
        ...
        # 70% de 240 puntos posibles (100 teóricos + 140 operativos esc. 1 al 14) es 168
        if puntos >= 168: 
            st.balloons()
        ...
        st.markdown("---")
        st.subheader("🗣️ Bloque 3: Cierre y Debate de Causa Raíz")
        st.info("**Pregunta del Taller:** ¿Cuál fue la clave para resolver la situación crítica en la planta?")
        st.text_area("Análisis escrito del Alumno (Monitoreo, tendencias, comunicación, trabajo en equipo):", key="debate_alumno_prod")
        
        st.markdown(f"""
        > 💡 *“En la operación real, las instalaciones hablan mediante presiones, niveles, temperaturas y caudales. El operador que sabe interpretar esas señales toma mejores decisiones antes de que aparezcan los problemas.”*
        """)
    ...
    with tab_teoria:
        ...
        st.markdown("#### 3. Ingeniería de Producion")
    ...
    with tab_simulador:
        st.markdown("### Escenarios de Simulación en Planta de Proceso (10 pts c/u)")
        ...
        st.markdown("#### Escenario 1 - Inicio de turno")
        st.info("Llegás a la planta y un operador informa una vibración inusual en la bomba de transferencia.")
        r11 = st.radio("Selección de maniobra (E1):", ["A) Ignorar el comentario", "B) Pedir datos de vibración y revisar historial", "C) Parar inmediatamente la planta", "D) Esperar una alarma"], index=None, key="q11")
        
        st.markdown("#### Escenario 2 - La vibración aumenta")
        st.info("Se confirma un aumento gradual de vibración.")
        r12 = st.radio("Selección de maniobra (E2):", ["A) Analizar alineación y estado mecánico", "B) Aumentar velocidad", "C) Reducir presión del separador", "D) Ignorar"], index=None, key="q12")
        
        st.markdown("#### Escenario 3 - Problema de gas")
        st.info("El caudal de gas cae y la presión del separador aumenta.")
        r13 = st.radio("Selección de maniobra (E3):", ["A) Verificar válvula de control de gas", "B) Revisar tanque", "C) Revisar calidad del petróleo", "D) Revisar sistema contra incendios"], index=None, key="q13")
        
        st.markdown("#### Escenario 4 - Ruido extraño")
        st.info("Se escuchan golpeteos en una válvula.")
        r14 = st.radio("Selección de maniobra (E4):", ["A) Golpear la válvula", "B) Revisar tendencias operativas", "C) Ignorar", "D) Abrir todas las válvulas"], index=None, key="q14")
        
        st.markdown("#### Escenario 5 - Alta presión")
        st.info("La presión continúa aumentando.")
        r15 = st.radio("Selección de maniobra (E5):", ["A) Revisar causa", "B) Esperar", "C) Aumentar producción", "D) Desactivar alarmas"], index=None, key="q15")
        
        st.markdown("#### Escenario 6 - Uso del bypass")
        st.info("La válvula principal responde lentamente.")
        r16 = st.radio("Selección de maniobra (E6):", ["A) Implementar bypass controlado", "B) Nada", "C) Forzar válvula", "D) Cerrar instalación"], index=None, key="q16")
        
        st.markdown("#### Escenario 7 - Nivel alto en separador")
        st.info("El nivel llega al 85%.")
        r17 = st.radio("Selección de maniobra (E7):", ["A) Verificar instrumentación de nivel", "B) Color del tanque", "C) Viento", "D) Iluminación"], index=None, key="q17")
        
        st.markdown("#### Escenario 8 - Arrastre de líquidos")
        st.info("Aparece líquido en la línea de gas.")
        r18 = st.radio("Selección de maniobra (E8):", ["A) Riesgo de daño aguas abajo", "B) Ninguno", "C) Menor consumo", "D) Mejor separación"], index=None, key="q18")
        
        st.markdown("#### Escenario 9 - Caída de producción")
        st.info("Un pozo cae de 150 a 90 m³/d.")
        r19 = st.radio("Selección de maniobra (E9):", ["A) Restricción", "B) Fantasmas", "C) Clima", "D) Ninguna"], index=None, key="q19")
        
        st.markdown("#### Escenario 10 - Falla eléctrica")
        st.info("Una bomba se detiene inesperadamente.")
        r20 = st.radio("Selección de maniobra (E10):", ["A) Revisar alimentación eléctrica", "B) Comprar otra", "C) Reiniciar varias veces", "D) Esperar"], index=None, key="q20")
        
        st.markdown("#### Escenario 11 - Alarma de alta presión")
        st.info("Se activa una alarma.")
        r21 = st.radio("Selección de maniobra (E11):", ["A) Confirmar lectura", "B) Ignorar", "C) Apagar alarma", "D) Continuar"], index=None, key="q21")
        
        st.markdown("#### Escenario 12 - Válvula reparada")
        st.info("Finalizó el mantenimiento.")
        r22 = st.radio("Selección de maniobra (E12):", ["A) Realizar prueba funcional", "B) Arrancar directo", "C) Ignorar procedure", "D) Desconectar instrumentos"], index=None, key="q22")
        
        st.markdown("#### Escenario 13 - Fin del evento")
        st.info("La situación vuelve a la normalidad.")
        r23 = st.radio("Selección de maniobra (E13):", ["A) Elaborar informe técnico", "B) Nada", "C) Borrar registros", "D) Olvidar incidente"], index=None, key="q23")
        
        st.markdown("#### Escenario 14 - Lecciones aprendidas")
        st.info("Documentación posterior al evento.")
        r24 = st.radio("Selección de maniobra (E14):", ["A) Evitar repetición", "B) Costumbre", "C) Archivo", "D) Sin motivo"], index=None, key="q24")
    ...
    if st.button("Finalizar Examen y Calificar", use_container_width=True, key="btn_calificar_final"):
        ...
        else: bitacora.append("Escenario 1: Ignorar la vibración reportada incrementó el riesgo de daño en la bomba.")
            
        if "A)" in r12: puntos_acumulados += 10
        else: bitacora.append("Escenario 2: No analizar la alineación aceleró la inestabilidad mecánica del sistema.")
            
        if "A)" in r13: puntos_acumulados += 10
        else: 
            bitacora.append("Escenario 3: No verificar la válvula de control (PCV) generó un pico de presión brusco.")
            st.session_state.motor.simular_golpe_de_gas()
            
        if "B)" in r14: puntos_acumulados += 10
        else: bitacora.append("Escenario 4: Los golpeteos requerían un análisis profundo de tendencias operativas.")
            
        if "A)" in r15: puntos_acumulados += 10
        else: 
            bitacora.append("Escenario 5: No buscar la causa raíz ante un aumento sostenido saturó la línea.")
            st.session_state.motor.simular_golpe_de_gas()
            
        if "A)" in r16: puntos_acumulados += 10
        else: bitacora.append("Escenario 6: La respuesta lenta de la válvula principal requería un bypass controlado.")
            
        if "A)" in r17: puntos_acumulados += 10
        else: bitacora.append("Escenario 7: Un nivel al 85% exige contrastar la instrumentación de medición campo-lazo.")
            
        if "A)" in r18: puntos_acumulados += 10
        else: bitacora.append("Escenario 8: El arrastre latente de líquidos expone a daños críticos a los compresores aguas abajo.")
            
        if "A)" in r19: puntos_acumulados += 10
        else: bitacora.append("Escenario 9: La caída abrupta de 150 a 90 m³/d sugería una restricción física o estrangulamiento.")
            
        if "A)" in r20: puntos_acumulados += 10
        else: bitacora.append("Escenario 10: La detención imprevista exigía el chequeo prioritario del lazo eléctrico.")
            
        if "A)" in r21: puntos_acumulados += 10
        else: bitacora.append("Escenario 11: Ante alarmas de alta presión, la primera regla es validar la lectura.")
            
        if "A)" in r22: puntos_acumulados += 10
        else: bitacora.append("Escenario 12: Arrancar sin prueba funcional post-mantenimiento viola el protocolo de seguridad.")
            
        if "A)" in r23: puntos_acumulados += 10
        else: bitacora.append("Escenario 13: El cierre formal de un evento requiere obligatoriamente un informe técnico.")
            
        if "A)" in r24: puntos_acumulados += 10
        else: bitacora.append("Escenario 14: Documentar las lecciones aprendidas es la única vía para mitigar repeticiones.")
